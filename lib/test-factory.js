...
class TestFactory {
    constructor(outputDir,debug){
        ...
        if(this.debug) console.log('TestFactory Created');
        fs.lstat(this.outputDir,(err,stats)=>{
            if(err || !stats.isDirectory()){
                if(this.debug) console.log(`Creating Output Directory ${this.outputDir}`)
    ...
    run(file){
        return new Promise((resolve,reject)=>{
            if(this.debug) console.log(`Running for ${file}`);
                        ...
                        if(ast.program){
                            ...
                            classNodes.forEach(classNode=>{
                                ...
                                if(content !== ''){
                                    ...
                                    if(this.debug) console.log(`Writing File ${filePath}...`);
                                    fs.writeFile(filePath,content,(err)=>{
                                        if(err) console.error(err);
                                    });
                                }  else {
                                    console.log('Empty Test Output');
                                } 
                            });
                            console.log('Ran for file',file);
                            resolve(file);
                        } else {
                            console.log('Program Not Found By AST');
                            resolve('Program Not Found By AST')
                        }
                    } catch(err){
                        if(this.debug) console.error(err);
                        resolve(err);
