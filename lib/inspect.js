...
// Condition module.paths to include whichever directory we're in, plus node_modules if available
...
class Inspect {
    ...
    inspectClass(obj){
        ...
        try{
            ...
            this.output += '\n});\nafter(function(done){\n\tdone();\n});\n';
            const filePath = this.outputDir+'/'+`${obj.name}.test.js`;
            try {
                fs.writeFileSync(filePath,this.output);    
            } catch (e){
                console.error(`${symbols.error} Error Writing Tests File ${filePath}: `+e);
            }
            try{
                _class = undefined;
            } catch(e){
                console.error(`${symbols.error} Error While Deleting ${obj.name}`+e);
            }
        } catch(e){
            console.error(`${symbols.error} Error Initialing Class: ${obj.name} `+e);
        }
    }
    inspect(file){
